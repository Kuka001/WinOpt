...
# Сбор карты устройств PnP с использованием CIM (взамен Get-WmiObject для совместимости с PS 7)
$pnpMap = @{}
try {
...
# Отключение энергосбережения через интерфейсы CIM (WMI)
...
# Настройка пробуждения устройств через powercfg
try {
...
# Отключение энергосбережения сетевых адаптеров
try {
...
if(Get-Command Disable-NetAdapterPowerManagement -EA 0){
    ...
    foreach($a in $adps){
        ...
        try {
            # Проверяем, поддерживает ли адаптер управление питанием (исключает виртуальные порты вроде kdnic)
            $pm = $null
...
# Глобальное отключение энергосбережения в активной схеме питания ОС
try {
    ...
    # Отключение USB Selective Suspend (выборочная приостановка USB)
...
# Настройка параметров реестра для USB и Bluetooth-устройств
